...
struct CLIOptions {
  ...
  std::string config_path;                      // 配置文件
  std::string config_string;                    // 配置字符串（Json）
...
private:
   ...
   * @return 验证结果
   */
  static CLIResult validate_config(const CLIOptions &options);
   ...
   * @param verbose 是否详细输出
   */
  static void print_validation_result(const class ValidationResult &result,
                                      bool verbose);
   ...
   * @return 可以覆盖返回true
   */
  static bool confirm_overwrite(const std::string &filepath,
   ...
   */
  static std::string
  generate_default_output_path(const std::string &packet_name);
};
} // namespace rplc
