...
void test_end_to_end_single_packet() {
    ...
    auto serialize_result = serializer.serialize(buffer.data(), buffer.size(), 1, original_packet);
...
void test_end_to_end_multi_packet() {
    ...
    auto serialize_result = serializer.serialize(buffer.data(), buffer.size(), 1, original_a, original_b);
    assert(serialize_result.has_value());
...
void test_real_world_chunked_scenario() {
    ...
    for (size_t i = 0; i < std::max(packets_a.size(), packets_b.size()); ++i) {
        if (i < packets_a.size()) {
            ...
            auto result = serializer.serialize(buffer_a.data(), buffer_a.size(), i + 1, packets_a[i]);
        ...
        if (i < packets_b.size()) {
            ...
            auto result = serializer.serialize(buffer_b.data(), buffer_b.size(), i + 1, packets_b[i]);
            assert(result.has_value());
...
void test_error_recovery_continued_processing() {
    ...
    auto result1 = serializer.serialize(valid_frame1.data(), valid_frame1.size(), 1, packet1);
    auto result2 = serializer.serialize(valid_frame2.data(), valid_frame2.size(), 2, packet2);
    auto result3 = serializer.serialize(valid_frame3.data(), valid_frame3.size(), 3, packet3);
...
void test_multiple_processing_cycles() {
    ...
    for (int cycle = 0; cycle < 5; ++cycle) {
        SampleA packet{static_cast<uint8_t>(cycle + 1), 
        ...
        // Serialize
        std::vector<uint8_t> buffer(frame_size);
        auto serialize_result = serializer.serialize(buffer.data(), buffer.size(), cycle + 1, packet);
        assert(serialize_result.has_value());
