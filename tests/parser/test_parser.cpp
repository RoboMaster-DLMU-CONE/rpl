...
void test_chunked_data_processing()
{
    ...
    auto serialize_result = serializer.serialize(buffer.data(), buffer.size(), packet_a, packet_b);
...
void test_noise_resilience()
{
    ...
    auto serialize_result = serializer.serialize(valid_frame.data(), valid_frame.size(), packet);
    assert(serialize_result.has_value());
