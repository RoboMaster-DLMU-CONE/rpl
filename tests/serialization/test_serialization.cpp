...
void test_multi_packet_serialization() {
  ...
  constexpr size_t total_frame_size =
...
void test_frame_size_calculations() {
  ...
  auto size_by_cmd_a =
      serializer.frame_size_by_cmd(RPL::Meta::PacketTraits<SampleA>::cmd);
  auto size_by_cmd_b =
      serializer.frame_size_by_cmd(RPL::Meta::PacketTraits<SampleB>::cmd);
...
void test_buffer_size_error_handling() {
  ...
  auto result =
      serializer.serialize(small_buffer.data(), small_buffer.size(), packet);
